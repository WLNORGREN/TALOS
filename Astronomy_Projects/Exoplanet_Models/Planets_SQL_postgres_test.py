...
try:
    ...
    columns = [desc[0]
               for desc in cursor.description]  # Get column names for output
    ...
    query = f"UPDATE {table_name}  SET pl_letter = 'eee' WHERE rowid = 2;"
    cursor.execute(query)
    query = f"SELECT rowid, pl_letter FROM {table_name} ORDER BY rowid  LIMIT 4;"
    cursor.execute(query)
    ...
    query = f"INSERT INTO {table_name} (rowid) VALUES (44444);"
    cursor.execute(query)
    query = f"SELECT rowid, pl_letter FROM {table_name} ORDER BY rowid DESC LIMIT 2;"
    cursor.execute(query)
    ...
    # Alter table
    query = f"ALTER TABLE {table_name} ADD galaxy CHAR"
    cursor.execute(query)
    query = f"SELECT rowid, pl_letter, galaxy FROM {table_name} ORDER BY rowid DESC LIMIT 2;"
    cursor.execute(query)
    ...
    cursor.execute(query)
    query = f"SELECT rowid, pl_letter, galaxy FROM {table_name} LIMIT 2;"
    cursor.execute(query)
