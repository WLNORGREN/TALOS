...
def graph(value=None):
    ...
    ax.text(0.2, 0.4, " Acceleration = $" + sp.latex(function(r)) + "$", fontsize=10) 
    
    ax.text(0.2, 0.6, "Electric Field Strength = " + str((1/(4*scp.constants.pi*scp.constants.epsilon_0))*int(s1.get())/(s2.get()**2)) + " $V/{m}$", fontsize=10) 
