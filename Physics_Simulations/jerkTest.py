...
def graph(value=None):
    ...
    ax.text(0.2, 0.1,"Snap: s(t) = $" + sp.latex(snap(t))+ "$ $\\frac{m}{s^4}$", fontsize=10) 
    canvas.draw()
    print("Test Phrase")
...
def plot(value=None):
    ...
    lam_x = sp.lambdify(t, snap(t), modules=['numpy'])
    vfunc = np.vectorize(lam_x)
    bx.plot(x_vals,vfunc(x_vals),color='orange')
